...
# Fresh installs intentionally have no wallet. Knots can sync without one.
if ! rpc listwalletdir | grep -Fq "\"name\": \"$WALLET_NAME\""; then
  echo "No Knots wallet configured yet; leaving payout unconfigured."
  rm -f "$ACTIVE_OUT" "$NODE_OUT" "$MODE_FILE"
  echo "KNOTS_WALLET_INIT=NO_WALLET"
  exit 0
fi
...
case "$MODE" in
  0|false|off|no)
    if [ -s "$EXTERNAL_OUT" ]; then ACTIVE_ADDRESS="$(cat "$EXTERNAL_OUT")";
    else printf "1\n" > "$MODE_FILE"; ACTIVE_ADDRESS="$NODE_ADDRESS"; fi
    ;;
